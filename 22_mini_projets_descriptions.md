...
Pour trouver la valeur de $\theta$ en fonction du temps, on pourra utiliser la [méthode d'Euler](https://en.wikipedia.org/wiki/Euler_method) de résolution d'équation différentielle. La formule ci-dessus donne l'accélération angulaire $a_{\theta} = \frac{d^2\theta}{dt^2}$. À partir de celle-ci, la méthode d'Euler propose le calcul de la vitesse angulaire $v_{\theta} = a_{\theta} * \delta t$ (où $\delta t$ représente le pas de temps entre deux étapes successives de la simulation). Enfin, cette vitesse $v_{\theta}$ donne l'angle $\theta$ lui même : $\theta = v_{\theta} * \delta t$. On prendra un pas de temps $\delta t$ = 0.05 s, une accélération gravitationnelle $g$ = 9.8 m.s$^{-2}$ et une longueur de tige de $l$ = 1 m.
